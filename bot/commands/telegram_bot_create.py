...
@Client.on_message(filters.private & filters.command("create", "/"))
@pyrogram_on_error
async def pyrogram_create(client, message):
    if await Users.filter(user=message.from_user.id).first():
...
async def pyrogram_code(client, message):
    ...
    await Users.create(user=user_data.id, token=encrypt_data(token))
